...
def get_bool(value: str, default: bool = False) -> bool:
    """Parse boolean env values safely."""
    if value is None:
        return default
...
class BettingConfig:
    ...
    min_bet: float = float(os.getenv("MIN_BET", 10.0))
    max_bet: float = float(os.getenv("MAX_BET", 10000.0))
    default_odd_type: str = os.getenv("DEFAULT_ODD_TYPE", "decimal")
...
class GameConfig:
    ...
    starting_stake: float = float(os.getenv("STARTING_STAKE", 1000.0))
    win_threshold: float = float(os.getenv("WIN_THRESHOLD", 2000.0))
    loss_threshold: float = float(os.getenv("LOSS_THRESHOLD", 100.0))
    auto_play_count: int = int(os.getenv("AUTO_PLAY_COUNT", 10))

    default_game_type: GameType = GameType[
        os.getenv("DEFAULT_GAME_TYPE", "ROULETTE").upper()
    ]
...
class LoggingConfig:
    ...
    log_to_console: bool = get_bool(os.getenv("LOG_TO_CONSOLE"), True)
    log_to_file: bool = get_bool(os.getenv("LOG_TO_FILE"), True)
